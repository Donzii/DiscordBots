...
def init():
	global readbuffer, send_queue, disconnect_time, silent, lastsend, lastconnect, connected, users, lastmessagetime, lastping
...
def run(frametime):
	global readbuffer, connected, lastconnect, disconnect_time, lastmessagetime
	
	if not connected:
		if frametime - lastconnect > 5:
			lastconnect = frametime
			lastmessagetime = frametime
			connect()
		...
		#if no messages recieved for 5 minuts, try to send a ping probes
		elif frametime-lastmessagetime > 360:
			if frametime-lastmessagetime < 375 and frametime-lastping > 4:
				send_queue.append("PING :probe\r\n")
				lastping = frametime
			else:
				console.display("CONNECTION ERROR: PING answer timeout!")
				reconnect()
